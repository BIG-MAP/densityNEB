...
class Interpolator():
    def __init__(self, grid: torch.Tensor, cell: ase.cell.Cell):
        ...
        # Images processing follows the convention that axes are on the
        # interval [-1, +1] and -1 and +1 are distinct points. By adding a
        # circular padding of 1 we get the correct scaling of the grid on the
        # open interval [0, 1) to the grid on the closed interval [0,1]
        self.padded_grid = torch.nn.functional.pad(
            grid, (0, 1)*3, mode="circular"
        )

        self.inv_cell_T = torch.tensor(np.linalg.inv(
            np.transpose(cell.complete())
    ...
    def __call__(self, input_points: torch.Tensor):
        ...
        scaled_pos = torch.remainder(scaled_pos, 1.0)
        ...
        for b in range(scaled_pos.shape[0]):
            interp_vals = torch.nn.functional.grid_sample(grid, torch.unsqueeze(
                scaled_pos[b], 0
