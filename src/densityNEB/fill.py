...
def highest(previous_cycle_data: dict, count: int, key: str, threshold=0.1):
    ...
    assert (dense_data.shape[0] - 1) % (path_coords.shape[0] - 1) == 0, f"Bad shape of dense data {dense_data.shape}."  # noqa
    assert float("nan") not in dense_data

    interpolate_count = int(
        (dense_data.shape[0] - 1) / (path_coords.shape[0] - 1) - 1
    )
    scores = path_coords.new(
        path_coords.shape[0] - 1, interpolate_count
    ).zero_()
...
def fill_chain(
    existing_chain: Tensor, insert_alphass: list,
    relative_lengths: Tensor = None
):
    """
    Extend a chain of coordinates by inserting additional items
    (through linear interpolation).

    :param existing_chain: The current chain
    :param insert_alphass: A list of float values in range (0, 1) specifying
    the relative position between each pair of pivots.
    :param relative_lengths: The current relative lengths between pivots. New
    relative lengths are only returned when a tensor is passed.
    :return:
    """
    existing_count = existing_chain.shape[0]
    assert len(insert_alphass) == existing_count - 1, "For each connection in line, a number of new items must be specified!"  # noqa
