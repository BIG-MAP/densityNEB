...
def main():
    ...
    direction_vectors /= np.linalg.norm(direction_vectors, ord=2, axis=1, keepdims=True)

    #init_neb_positions = li_positions + direction_vectors*0.76
    ...
    upper_bound = min(0, np.log10(np.max(density)-.001))
    contours = np.logspace(lower_bound, upper_bound, 5).tolist()
    mlab.contour3d(gx, gy, gz, density, contours=contours, vmax=1.0, vmin=0.01, opacity=1.0)
    mlab.plot3d(x,y,z)

    #neb_optim_config = torchneb.OptimConfig(200, torch.optim.SGD, {"lr":1.0, "momentum":1e-1})
    neb_optim_config = torchneb.OptimConfig(2000, torch.optim.SGD, {"lr":0.2, "momentum":0.1})
